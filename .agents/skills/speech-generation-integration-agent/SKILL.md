...
description: >-
  Integrate paper evidence into wiki/_claims YAML and synthesize claim clusters and method
  families without rendering pages. Use after relevant papers are ingested or claims are stale.
---
...
- **Wiki content repo**: the path returned by `python3 scripts/resolve_wiki_dir.py`
...
Before logging, determine `RUNTIME`, `PROVIDER`, and the exact `MODEL` using
`docs/schemas/generation.md`. Use `unknown` only when the runtime exposes no model identifier.
...
bullet = f'- integrate | {n_papers} papers | {n_concepts} concepts updated | {n_clusters} clusters updated | {n_reassessed} reassessments checked | runtime: {runtime} | provider: {provider} | model: {model}'
section = f'## {today}'
path = f'{WIKI}/log.md'
text = open(path).read()
if section in text:
