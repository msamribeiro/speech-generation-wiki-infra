...
description: >-
  Score pending metadata records for speech-generation relevance, assign corpus status and task
  fields, queue borderline papers, and log the run. Use when pending papers need filtering.
---
...
Determine `RUNTIME`, `PROVIDER`, and the exact `MODEL` using `docs/schemas/generation.md` before
logging. Use `unknown` only when the runtime exposes no model identifier.

```
- filter | {source description} | {N} accepted, {M} review, {K} rejected | runtime: {runtime} | provider: {provider} | model: {model}
```
