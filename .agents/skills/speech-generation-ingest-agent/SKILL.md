...
description: >-
  Create a Tier 1 wiki paper page from one accepted parsed paper, update indexes, logs, and
  metadata, and emit the ingest result. Use when asked to ingest a single eligible paper ID.
---
...
Record the result as `COMMIT`. Determine `RUNTIME`, `PROVIDER`, and `MODEL` using
`docs/schemas/generation.md`: runtime is `claude-code` or `codex`, provider is `anthropic` or
`openai`, and model is the exact ID exposed by the runtime or `unknown`. Never copy an example ID.
Use all four values in page frontmatter, the changelog entry, and metadata history. Set `op` to
`re-ingest` if `generation_history` is non-empty; otherwise `ingest`.
...
Write the full paper page following the exact template below. Write to the content repo path — `WIKI="$(python3 scripts/resolve_wiki_dir.py)"`. Read the paper carefully — synthesise the prose sections; do not copy the abstract.
...
generation:
  schema_version: 2
  date: {TODAY}
  runtime: "{RUNTIME from step 2b}"
  provider: "{PROVIDER from step 2b}"
...
bullet = f\"- ingest | {meta['id']} | {meta.get('title','')} | {meta.get('venue','arXiv')} {meta.get('year','')} | runtime: {runtime} | provider: {provider} | model: {model}\"
section = f'## {today}'
...
runtime = 'RUNTIME_PLACEHOLDER'
provider = 'PROVIDER_PLACEHOLDER'
entry = {'schema_version': 2, 'date': date.today().isoformat(), 'op': op, 'runtime': runtime, 'provider': provider, 'agent': 'speech-generation-ingest-agent', 'model': model, 'commit': commit}
