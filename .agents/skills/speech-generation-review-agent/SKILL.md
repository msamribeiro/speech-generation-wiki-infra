...
description: >-
  Audit and correct an existing Tier 1 wiki page against the full parsed paper while preserving
  valid prose and provenance. Use when asked to review one ingested paper ID.
---
...
Record as `COMMIT`. Determine `RUNTIME`, `PROVIDER`, and `MODEL` using
`docs/schemas/generation.md`. Use the exact model ID exposed by the runtime or `unknown`; never
copy an example ID. Use these values in the corrected page, changelog, and metadata history.
...
bullet = f\"- review | {meta['id']} | {meta.get('title','')} | {meta.get('venue','arXiv')} {meta.get('year','')} | runtime: {runtime} | provider: {provider} | model: {model}\"
section = f'## {today}'
...
runtime = 'RUNTIME_PLACEHOLDER'
provider = 'PROVIDER_PLACEHOLDER'
entry = {'schema_version': 2, 'date': date.today().isoformat(), 'op': op, 'runtime': runtime, 'provider': provider, 'agent': 'speech-generation-review-agent', 'model': model, 'commit': commit}
