...
description: >-
  Create a lightweight wiki stub for one eligible Tier 2 citation-discovery paper and update
  indexes, logs, and metadata. Use when asked to ingest a Tier 2 paper ID.
---
...
Use all four values in page frontmatter, the changelog entry, and metadata history.
...
runtime = 'RUNTIME_PLACEHOLDER'
provider = 'PROVIDER_PLACEHOLDER'
entry = {'schema_version': 2, 'date': date.today().isoformat(), 'op': op, 'runtime': runtime, 'provider': provider, 'agent': 'speech-generation-lightweight-ingest-agent', 'model': model, 'commit': commit}
