...
- [ ] Run integration passes for all supported concepts — per-concept, two-phase model. Dedicated session note at `Q3_INTEGRATION_SESSIONS.md` (scope: all papers published before Q4 2025, across all 23 concepts). **`flow-matching` and `speech-to-speech` fully closed** (Phase 1 + Phase 2) as of 2026-07-20: 97/97 and 60/63 Q3-scoped papers respectively (the 3-paper gap on speech-to-speech is intentional — 3 papers correctly excluded for having no speech-generation stage of their own). 21 other concepts have not started — no `_claims/` YAML exists for them yet. Q3-scoped `papers_not_in_any_yaml`: 429 as of 2026-07-20 (corrected count — see session note for a Tier-2-detection bug found and fixed in the scoping scripts used to produce concept-level counts)
...
- [ ] Standardize `related_concepts` frontmatter serialization across all paper pages — found 2026-07-19 while building a Q3-scoped per-concept integration coverage table. Three different YAML serializations coexist across the 640 pages, with no clean before/after cutover (23 separate ingest days each contain a mix, so it's per-invocation agent drift, not a one-time format change): bracket unquoted `[flow-matching, zero-shot-tts]` (286 pages), bracket quoted `["flow-matching", "zero-shot-tts"]` (348 pages), and YAML block-list (`related_concepts:` / `  - flow-matching`) (6 pages). All three are valid YAML and parse identically under `yaml.safe_load` — `health_check.py` and `corpus_summary.py` already parse the frontmatter as real YAML, so pipeline tooling is unaffected. The risk is any future hand-rolled script or manual grep assuming a single format: an ad-hoc script built for this exact table undercounted flow-matching's Q3-scoped paper count by roughly half (51 vs. the true 97) before the bug was found. Fix: (a) pick one canonical serialization (bracket, unquoted, matches the majority-recent style) and document it in `docs/schemas/metadata.md`; (b) write a one-time normalization script to rewrite all 640 pages to match; (c) update ingest/lightweight-ingest/review agent specs to always emit the canonical form.
