...
- [ ] Run integration passes for all supported concepts — per-concept, two-phase model. Dedicated session note at `Q3_INTEGRATION_SESSIONS.md` (scope: all papers published before Q4 2025, across all 23 concepts). **`flow-matching`, `speech-to-speech`, `rlhf-speech`, `evaluation-metrics`, `disentanglement`, `autoregressive-codec-tts`, `instruction-conditioned-tts`, and `prosody-control` are fully closed** (Phase 1 + Phase 2): 97/97, 60/63, 29/29, 285/286, 100/100, 165/165, 44/44, and 94/94 Q3-scoped papers respectively (the 3-paper gap on speech-to-speech and the 1-paper gap on evaluation-metrics are intentional exclusions). `prosody-control` closed on 2026-07-25 with 14 method_families and 17 claim_clusters from 404 Phase 1 claims; 82/94 papers have reciprocal family assignments and 12 low-relevance/contextual outliers remain. `instruction-conditioned-tts` closed the same day with 8 method_families and 15 claim_clusters. `autoregressive-codec-tts` closed with 13 method_families and 17 claim_clusters. `evaluation-metrics` remains the largest closed concept (285 papers); `autoregressive-codec-tts` is second (165), followed by `disentanglement` (100). 15 other concepts have not started — no `_claims/` YAML exists for them yet. Corpus-wide `papers_not_in_any_yaml`: 99 as of 2026-07-25.
