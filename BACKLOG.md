...
- [ ] Run integration passes for all supported concepts — per-concept, two-phase model. Dedicated session note at `Q3_INTEGRATION_SESSIONS.md` (scope: all papers published before Q4 2025, across all 23 concepts). **`flow-matching`, `speech-to-speech`, `rlhf-speech`, and `evaluation-metrics` fully closed** (Phase 1 + Phase 2) as of 2026-07-21: 97/97, 60/63, 29/29, and 285/286 Q3-scoped papers respectively (the 3-paper gap on speech-to-speech and the 1-paper gap on evaluation-metrics are both intentional — papers correctly excluded for lacking a speech-generation stage / being off-topic). `evaluation-metrics` is the largest concept closed so far (285 papers, 17 method_families, 31 claim_clusters). 19 other concepts have not started — no `_claims/` YAML exists for them yet. Q3-scoped `papers_not_in_any_yaml`: 205 as of 2026-07-21 (down from 411 on 2026-07-20)
