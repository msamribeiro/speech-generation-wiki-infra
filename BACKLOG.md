...
- [ ] Run review pass on all papers flagged by `health_check.py --module ingest` — use the `speech-generation-review-agent` to correct frontmatter, add missing `field_significance` and `## Claims` sections with `*(§N.N)*` citations, embed warranted architecture figures, and fix any other structural gaps against the full expected page template; currently known targets: ~95 papers missing `field_significance`, ~56 papers with uncited claim bullets; run health_check first to get the authoritative list; review one at a time with quality check between papers — **IN PROGRESS (2026-06-23): 32 papers reviewed this session (batches of 4); 72 remaining (0 Interspeech — all done, 17 NLP, 55 arXiv); see REVIEW_SESSION.md**
