...
- review | 2507.14815 (FastLongSpeech: Enhancing Large Speech-Language Models for Efficient Long-Speech Processing) | 1 paper | rejected | caught at ingest time (Q4 session 16, batch 1) before any page was written; speech-comprehension-efficiency paper (Qwen2-Audio + speech-input compression) whose own output is always text (§2 Eq. 1), every benchmark scores text/label accuracy (Spoken QA, AIR-Bench, MELD, ASR WER, SPIRAL-H), only TTS mention (Orca) synthesizes benchmark input audio, not model output; clean FAMA/MLC-SLM/2510.12116 match, user confirmed reject | status: accepted → rejected, review_queue.md decision recorded

## 2026-07-18

- review | 2510.12116 (Understanding the Modality Gap: An Empirical Study on the Speech-Text Alignment Mechanism of Large Speech Language Models) | 1 paper | undecided, skipped | caught at ingest time (Q4 session 15 batch 3) before any page was written; the LSLM under study only ever outputs text (§3.1: "autoregressive generation of textual responses"), all evaluation is VoiceBench QA-accuracy subsets (AdvBench, IFEval, OBQA, MMSU, sd-qa), no TTS/VC/spoken-output quality metric anywhere in the paper; cleaner FAMA/MLC-SLM match than 2510.09424 (no dialogue-state or any spoken-output component at all, vs. DST's structured JSON output within a spoken-dialogue pipeline) | status: accepted (unchanged), not ingested, flagged in review_queue.md for a later decision
- review | 2510.09424 (The Speech-LLM Takes It All: A Truly Fully End-to-End Spoken Dialogue State Tracking Approach) | 1 paper | accepted (explicit scope override) | caught at ingest time; system is a speech encoder + connector + LLM (with attention-pooling context-compression module) that consumes a full spoken multi-turn dialogue and emits a structured JSON of DST slot-value pairs, evaluated via Joint Goal Accuracy on SpokenWOZ; no TTS/VC/synthesized speech output anywhere, sole generative step is the LLM emitting JSON — structurally matches the FAMA (2025.clicit-1.81) / MLC-SLM (2509.13785) "understanding task wearing speech-LLM terminology" reject pattern; distinct from the AURA/VoiceAgentBench precedent (incidental TTS/VC building benchmark input audio), since here there is no TTS/VC component at all — user explicitly overrode the scope concern and accepted anyway, citing architectural/methodological relevance (context management for long spoken multi-turn input into an LLM); logged as a one-off override, not a new precedent for future DST-only papers | status: accepted → ingested, review_queue.md decision recorded
...
- review | 2602.23333 (SemanticVocoder: Bridging Audio Generation and Audio Understanding via Semantic Latents) | 1 paper | flagged for review | caught at ingest time before any page was written; paper is a general (non-speech) text-to-audio generation system trained on AudioSet and evaluated on AudioCaps/Clotho/WavCaps/HEAR sound-event benchmarks, with no speech/speaker/prosody/linguistic content anywhere; filter task: [TTS, codec] appears to be a keyword false positive (TTA vs TTS, generic "vocoder" match); new scope-failure shape (non-speech generation, not understanding-wearing-generative-framing), no clean precedent match, so left pending rather than auto-rejected | status: accepted -> review, review_queue.md entry recorded
- review | 2602.23333 (SemanticVocoder) | 1 paper | rejected | user confirmed reject: out of corpus scope, general (non-speech) text-to-audio generation, establishes a new precedent shape distinct from the FAMA/MLC-SLM pattern | status: review -> rejected, review_queue.md decision recorded
- review | 2602.23765 (DashengTokenizer: One layer is enough for unified audio understanding and generation) | 1 paper | flagged for review | caught at ingest time (Q1 2026 session batch 2, second candidate in batch), before any page was written; paper is an explicitly tri-domain (speech/music/environmental-sound) unified audio tokenizer — ~71% of training data is speech and it reports a dedicated SEED-TTS speech reconstruction benchmark against TTS-community codecs (Mimi, XCodec 2.0, SNAC, XY-Tokenizer) plus speech enhancement and speech-understanding tasks, but every generative demonstration (TTA on AudioCaps, TTM on MusicCaps, speech enhancement/denoising) is non-TTS, non-VC; the paper never performs text-to-speech or voice conversion anywhere. New scope-failure shape distinct from the clean SemanticVocoder reject (zero speech content) — this one has substantial speech content but is not speech-scoped as a generative system, so left pending for explicit decision rather than assumed either way | status: accepted -> review, review_queue.md entry recorded
- review | 2602.23765 (DashengTokenizer) | 1 paper | accepted | user confirmed accept: SEED-TTS speech-reconstruction benchmark against known in-corpus TTS codecs is genuine neural-codec subject matter even without a TTS/VC generation component; scope to task: [codec] only | status: review -> accepted, review_queue.md decision recorded
- review | 2603.00958 (S-VoCAL: A Dataset and Evaluation Framework for Inferring Speaking Voice Character Attributes in Literature) | 1 paper | flagged for review | caught at ingest time (Q1 2026 session batch 2, third candidate in batch), before any page was written; the paper's Attribute Inference task is entirely text-in/text-out (novel text + Wikidata-derived ground truth -> RAG pipeline with E5-large retrieval and Qwen3-8B/Phi-4-14B LLM inference predicting 8 categorical/text character attributes); no audio is produced, consumed, or evaluated anywhere, no TTS/VC system is trained or benchmarked, and all reported metrics (Weighted F1, soft-F1, Cohen's kappa, Krippendorff's alpha, Qwen3-embedding similarity/HAS) are text-attribute-matching metrics, not speech-quality metrics; filter task: [TTS, evaluation] is a false positive from TTS-motivation language in the abstract, not genuine subject-matter overlap; cleaner reject than the FAMA/MLC-SLM/2510.12116 precedents since even the inputs are text, not audio | status: accepted -> review, review_queue.md entry recorded
- review | 2603.00958 (S-VoCAL) | 1 paper | rejected | pure text-in/text-out NLP benchmark, no audio anywhere in the pipeline, cleaner reject than the FAMA/MLC-SLM precedent it matches | status: review -> rejected, review_queue.md decision recorded
- review | 2603.01592 (TQCodec: Towards neural audio codec for high-fidelity music streaming) | 1 paper | rejected | caught at ingest time (Q1 2026 session batch 3, second candidate in batch), before any page was written; trains and evaluates exclusively on music datasets (MusDBHQ, Jamendo, 100k+-track private music corpus per Table 2) with LSD/SNR metrics only, no speech-domain training or evaluation data, and no comparison to any speech codec anywhere in the paper; clean match to the MIDI-VALLE (2507.08530) and SemanticVocoder (2602.23333) reject shape, does not meet the DashengTokenizer (2602.23765) narrow-accept bar (no speech-reconstruction benchmark against in-corpus speech codecs) | status: accepted -> rejected, review_queue.md entry recorded
- review | 2603.04145 (VietNormalizer: An Open-Source, Dependency-Free Python Library for Vietnamese Text Normalization in TTS and NLP Applications) | 1 paper | flagged retroactively | caught after the ingest agent wrote the page directly rather than stopping first (Q1 2026 session batch 3, fourth/final candidate); paper is explicitly TTS-pipeline-framed (title, abstract, S1, S4, S5.3) but reports zero quantitative results of any kind (no normalization accuracy, no latency, no downstream TTS/MOS/WER evaluation, only a qualitative feature-comparison table), a materially weaker empirical case than the 2510.03111 precedent it invoked (which had real signal-quality metrics); user confirmed keep, judging TTS-pipeline framing plus open-source code availability genuine infrastructure value even without benchmarks | status: accepted -> ingested (unchanged), review_queue.md entry recorded retroactively as a new precedent

## 2026-08-23

- filter | cs.SD+eess.AS arXiv top-up (--date-from 2026-05-25, 213 written) + cs.CL OAI-PMH top-up (--date-from 2026-05-25, 40 written) | 253 pending papers scored | 185 accepted, 35 review, 33 rejected | accept rate 73% | runtime: claude-code | provider: anthropic | model: claude-sonnet-5
- filter | ACL Anthology 2026 top-up (main track + co-located workshops: SIGUL, *SEM, SwissText, TACL, LREC, SIGDIAL, PROPOR, IWSDS, IWSLT, AmericasNLP, AfricaNLP, DravidianLangTech, LoResLM, CAS, DialRES, NLPAICS, AbjadNLP; `scripts/fetch/acl.py --years 2026 --all-workshops`) | 49 pending papers scored | 31 accepted, 8 review, 10 rejected | accept rate 63% | runtime: claude-code | provider: anthropic | model: claude-sonnet-5
